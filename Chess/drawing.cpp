...
/*
*   libreria aggiunta per la riga 656 (potrebbe cambiare la riga):
*   std::wstring_convert<std::codecvt_utf8_utf16<wchar_t>> converter;
* 
* 
*   e anche per "converter.from_bytes(IMAGE_PATH).c_str()" in (riga 661):
*   HICON icon = (HICON)LoadImage(NULL, converter.from_bytes(IMAGE_PATH).c_str(), IMAGE_ICON, 0, 0, LR_LOADFROMFILE);
*/
#include <codecvt>
...
void DrawCircle(double x, double y, double radius, Color c, bool filled) {
    lock_guard<mutex> lock(bitmapLock);
    if (!graphics) return;
...
void Draw(string fileName, int x, int y) {
    wstring wsFilwName(fileName.begin(), fileName.end());
    Gdiplus::Bitmap bitmap(wsFilwName.c_str());
    // https://stackoverflow.com/questions/11684455/how-to-draw-transparent-bmp-with-gdi
    Gdiplus::Color ColourOfTopLeftPixel;
    Gdiplus::Status eStatus = bitmap.GetPixel(0, 0, &ColourOfTopLeftPixel);
    ...
    imageAtt.SetColorKey(ColourOfTopLeftPixel, ColourOfTopLeftPixel, Gdiplus::ColorAdjustTypeBitmap);
    lock_guard<mutex> lock(bitmapLock);
    if (!graphics) return;
    // Gdiplus::Image test(TEXT("D:\\Images\\image.bmp"));
    // Gdiplus::PointF current;
    // current.X = x;
    // current.Y = y;
    // graphics->DrawImage(&bitmap, current);
    // https://docs.microsoft.com/it-it/windows/win32/gdiplus/-gdiplus-using-a-color-matrix-to-set-alpha-values-in-images-use?redirectedfrom=MSDN
    INT iWidth = bitmap.GetWidth();
    INT iHeight = bitmap.GetHeight();
    graphics->DrawImage(&bitmap,
        Gdiplus::Rect(x, y, iWidth, iHeight),  // Destination rectangle
...
int WINAPI WinMain(HINSTANCE instance, HINSTANCE, LPSTR, int cmdShow) {
    ...
    // Calcola le coordinate iniziali per centrare la finestra
    int windowWidth = r.right - r.left;
    int windowHeight = r.bottom - r.top;
    int windowX = (screenWidth - windowWidth) / 2;
    int windowY = (screenHeight - windowHeight) / 2;

    HWND wnd = CreateWindowW(L"Immediate2D", L"Chess", style, windowX, windowY, r.right - r.left, r.bottom - r.top, nullptr, nullptr, instance, nullptr);
    if (wnd == nullptr) return 1;
    
    // istanzion un converter da stringa a lpcwstr (wstring)
    std::wstring_convert<std::codecvt_utf8_utf16<wchar_t>> converter;

    // Imposto l'icona della finestra manualmente
    HICON icon = (HICON)LoadImage(NULL,
        converter.from_bytes(IMG_PATH + "chess_icon.ico").c_str(), /* converto la stringa in LPCWSTR utilizzando il converter */
